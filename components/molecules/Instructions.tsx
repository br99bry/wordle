...
const Instructions = ({ handleClick }) => {
  ...
  return (
    <main className="flex items-center flex-col	">
      ...
      <div className="flex gap-3 w-full justify-center mb-2">
        {
          example1.map((item, index) => (
            <LetterCard customClass={`${index === 0 ? 'bg-hard-green border-hard-green' : 'border-black'} rounded-hard w-box-76 h-box-76 border `} key={index}>
              <span className="text-sub-big text-black">
                {item}
              </span>
            </LetterCard>))
        }
      ...
      <div className="flex gap-3 w-full justify-center mb-2">
        {
          example2.map((item, index) => (
            <LetterCard customClass={`${index === 2 ? 'bg-hard-yellow border-hard-yellow' : 'border-black'} rounded-hard w-box-76 h-box-76 border`} key={index}>
              <span className="text-sub-big text-black">
                {item}
              </span>
            </LetterCard>))
      ...
      <div className="flex gap-3 w-full justify-center mb-2">
        {
          example3.map((item, index) => (
            <LetterCard customClass={`${index === 4 ? 'bg-hard-gray border-hard-gray' : 'border-black'} rounded-hard w-box-76 h-box-76 border`} key={index}>
              <span className="text-sub-big text-black">
                {item}
              </span>
            </LetterCard>))
      ...
      <Button isDisabled={false} handleClick={handleClick} customClass="w-fit text-mid-bold text-white bg-hard-green rounded-soft py-2 px-12">!JUGAR¡</Button>
    </main>
  )
}
